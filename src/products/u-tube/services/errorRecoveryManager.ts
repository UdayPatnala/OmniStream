  ...
  public handlePlayerError(errorEvent: any): Video | null {
    ...
    if (nextVideo) {
      store.setRecoveryMessage(
        `âš ï¸ Playback error (${errorCode}). Auto-switched to candidate #${store.currentCandidateIndex + 1}: "${nextVideo.title.slice(0, 40)}..."`
      );
      ...
      store.setRecoveryMessage('âŒ All candidates exhausted for this topic. Please try searching another term.');
      return null;
    }
  }
