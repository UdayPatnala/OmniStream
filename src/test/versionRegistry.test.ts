...
describe('OmniStream Hierarchical Version Registry', () => {
  it('maintains authoritative Level 1 release version matching v1.5.0', () => {
    expect(VERSION_REGISTRY.version).toBe('1.5.0');
  ...
  it('contains valid Level 2 product modules for CineMorph and U-Tube', () => {
    ...
    expect(VERSION_REGISTRY.products.cinemorph.version).toBe('1.5.0');
    expect(VERSION_REGISTRY.products.utube.code).toBe('UT');
    expect(VERSION_REGISTRY.products.utube.version).toBe('1.5.0');
  });
  ...
  it('generates accurate formatted version summaries and diagnostic payloads', () => {
    ...
    expect(summary).toBe('OmniStream v1.5.0 [CM:v1.5.0, UT:v1.5.0]');

    const diagnostics = getVersionDiagnostics();
    expect(diagnostics['OmniStream Release']).toContain('v1.5.0');
    expect(diagnostics['CineMorph Product']).toBe('v1.5.0');
    expect(diagnostics['U-Tube Product']).toBe('v1.5.0');
    expect(diagnostics['Subsystem (CM-SF)']).toBe('Adaptive Smart Framing Engine v1.0.0');
