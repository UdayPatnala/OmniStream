...
describe('OmniStream Hierarchical Version Registry', () => {
  it('maintains authoritative Level 1 release version matching v1.8.0', () => {
    expect(VERSION_REGISTRY.version).toBe('1.8.0');
    expect(VERSION_REGISTRY.status).toBe('STABLE_RELEASE');
  });
  ...
  it('contains valid Level 2 product modules for CineMorph and U-Tube', () => {
    ...
    expect(VERSION_REGISTRY.products.cinemorph.version).toBe('1.8.0');
    expect(VERSION_REGISTRY.products.utube.code).toBe('UT');
    expect(VERSION_REGISTRY.products.utube.version).toBe('1.8.0');
  });
  ...
  it('contains core independent CineMorph subsystems', () => {
    ...
    expect(subsystems?.length).toBe(6);

    const codes = subsystems?.map((s) => s.code);
    expect(codes).toContain('CM-SF');
    expect(codes).toContain('CM-PERCEPT');
  ...
  it('generates accurate formatted version summaries and diagnostic payloads', () => {
    ...
    expect(summary).toBe('OmniStream v1.8.0 [CM:v1.8.0, UT:v1.8.0]');

    const diagnostics = getVersionDiagnostics();
    expect(diagnostics['OmniStream Release']).toContain('v1.8.0');
    expect(diagnostics['CineMorph Product']).toBe('v1.8.0');
    expect(diagnostics['U-Tube Product']).toBe('v1.8.0');
    expect(diagnostics['Subsystem (CM-SF)']).toBe('Adaptive Smart Framing Engine v1.8.0');
    expect(diagnostics['Subsystem (CM-PERCEPT)']).toBe('Universal Visual & Audio Perception System v1.8.0');
    ...
    expect(diagnostics['Subsystem (CM-VQ)']).toBe('Video & Frame Quality Intelligence v1.8.0');
    expect(diagnostics['Subsystem (CM-DSP)']).toBe('5-Band Parametric Audio DSP & Dynamic Clarity v1.8.0');
  });
});
